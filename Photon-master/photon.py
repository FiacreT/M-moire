...
class Photon(Resource):

    def intel_extractor(self, url, response):
    ...
    def extractor(self, url):
        ...
        for link in matches:
            ...
            if is_link(link, processed, files):
                if link[:4] == 'http':
                    ...
                    verb('Internal page', link)
                    internal.add(self.remove_file(url) + link)
                else:
                    verb('Internal page', link)
                    usable_url = self.remove_file(url)
        ...
        if not only_urls:
            self.intel_extractor(url, response)
            self.js_extractor(response)
    ...
    def get(self, target, level_):
        return results, 201

api.add_resource(Photon, "/photon/<string:target>/<string:level_>")
app.run(debug=True, port=5013, threaded=True)