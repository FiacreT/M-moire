...
class Dnsrecon(Resource):
    ...
    def process_spf_data(self, res, data):
        """
        ...
        for inc_ranges in includes:
            for spr_rec in res.get_txt(inc_ranges):
                spf_data = self.process_spf_data(res, spr_rec[2])
    ...
    def expand_cidr(self, cidr_to_expand):
        ...
        """
        ip_list = []
        c1 = self.IPNetwork(cidr_to_expand)
        return c1
    ...
    def expand_range(self, startip, endip):
        ...
        form the range.
        """
        return self.IPRange(startip, endip)
    ...
    def range2cidr(self, ip1, ip2):
        ...
        """
        r1 = self.IPRange(ip1, ip2)
        return str(r1.cidrs()[-1])


    def write_to_file(self, data, target_file):
        """
    ...
    def check_wildcard(self, res, domain_trg):
        """
    ...
    def check_nxdomain_hijack(self, nameserver):
        """
    ...
    def brute_tlds(self, res, domain, verbose=False):
        """
    ...
    def brute_srv(self, res, domain, verbose=False):
        """
    ...
    def brute_reverse(self, res, ip_list, verbose=False):
        """
    ...
    def brute_domain(self, res, dict, dom, filter=None, verbose=False, ignore_wildcard=False):
        """
    ...
    def in_cache(self, res, dict_file, ns):
        """
    ...
    def se_result_process(self, res, found_hosts):
        """
    ...
    def whois_ips(self, res, ip_list):
        """
        ...
        list = self.get_whois_nets_iplist(unique(ip_list))
        if len(list) > 0:
            ...
            if "a" in answer:
                for i in range(len(list)):
                    ...
                    found_records.append(self.brute_reverse(res, self.expand_range(list[i]['start'], list[i]['end'])))
                ...
                for a in answer:
                    ...
                    found_records.append(self.brute_reverse(res, self.expand_range(net_selected['start'], net_selected['end'])))
        else:
    ...
    def dns_record_from_dict(self, record_dict_list, scan_info, domain):
        """
    ...
    def write_json(self, jsonfile, data, scan_info):
        ...
        #self.write_to_file(json_data, jsonfile)
        return data


    def write_db(self, db, data):
        """
    ...
    def get_nsec_type(self, domain, res):
        target = "0." + domain

        answer = self.get_a_answer(res, target, res._res.nameservers[0], res._res.timeout)
    ...
    def dns_sec_check(self, domain, res):
        """
        ...
        try:
            ...
            nsectype = self.get_nsec_type(domain, res)
    ...
    def check_bindversion(self, res, ns_server, timeout):
        """
    ...
    def check_recursive(self, res, ns_server, timeout):
        """
    ...
    def general_enum(self, res, domain, do_axfr, do_google, do_bing, do_spf, do_whois, do_crt, zw):
        """
        ...
        self.check_wildcard(res, domain)
        ...
        if from_zt is None:
            ...
            try:
                for ns_rcrd in res.get_ns():
                    ...
                    # Save dictionary of returned record
                    recursive = self.check_recursive(res, ns_rcrd[2], res._res.timeout)
                    bind_ver = self.check_bindversion(res, ns_rcrd[2], res._res.timeout)
            ...
            if do_spf and len(text_data) > 0:
                ...
                processed_spf_data = self.process_spf_data(res, text_data)
                ...
                if len(found_spf_ranges) > 0:
                    ...
                    returned_records.extend(self.brute_reverse(res, unique(found_spf_ranges)))
                else:
            ...
            srv_rcd = self.brute_srv(res, domain)
            if srv_rcd:
                for r in srv_rcd:
            ...
            if do_google:
                ...
                goo_rcd = self.se_result_process(res, scrape_google(domain))
                if goo_rcd:
                    for r in goo_rcd:
            ...
            if do_crt:
                ...
                crt_rcd = self.se_result_process(res, scrape_crtsh(domain))
                for r in crt_rcd:
            ...
            if do_whois:
                whois_rcd = self.whois_ips(res, ip_for_whois)
                if whois_rcd:
                    for r in whois_rcd:
                        returned_records.extend(r)

            if zw:
                zone_info = self.ds_zone_walk(res, domain)
                if zone_info:
    ...
    def query_ds(self, res, target, ns, timeout=5.0):
        """
    ...
    def lookup_next(self, target, res):
        """
    ...
    def get_a_answer(self, res, target, ns, timeout):
    ...
    def get_next(self, res, target, ns, timeout):
        next_host = None
        response = self.get_a_answer(res, target, ns, timeout)
    ...
    def ds_zone_walk(self, res, domain):
        """
        ...
        try:
            while pending:
                ...
                records.extend(self.lookup_next(hostname, res))
                ...
                for transformation in transformations:
                    ...
                    if not nameserver:
                        response = self.get_a_answer(res, target, res._res.nameservers[0], timeout)
                    else:
                        response = self.get_a_answer(res, target, nameserver, timeout)
                    for a in response.authority:
                        if a.rdtype != 47:
                            continue
    ...
    def post(self, target):
        ...
        type="brt,axfr,std,brt,srv,tld,bing,crt,zonewalk"
        #type="brt"
        domain = target
        #arguments.ns_server = None
        #print("----------------------------------------------", domain)
        #exit()
        ...
        arguments.dictionary = "namelist.txt"
        ...
        if arguments.range:
            ip_list = self.process_range(arguments.range)
            if len(ip_list) > 0:
                if type is None:
                    type = "rvl"
        ...
        if type is not None:
            ...
            for r in type.split(','):
                ...
                try:
                    if r == "axfr":
                        ...
                        std_enum_records = self.general_enum(res, domain, xfr, goo, bing, spf_enum, do_whois, do_crt, zonewalk)
                        ...
                        if len(ip_list) > 0:
                            ...
                            rvl_enum_records = self.brute_reverse(res, ip_list, verbose)
                        ...
                        if (dict is not None) and (domain is not None):
                            ...
                            brt_enum_records = self.brute_domain(res, dict, domain, wildcard_filter, verbose, ignore_wildcardrr)
                            ...
                            if os.path.isfile(name_list_dic):
                                ...
                                brt_enum_records = self.brute_domain(res, name_list_dic, domain, wildcard_filter, verbose, ignore_wildcardrr)
                                if (output_file is not None) or (results_db is not None) or (csv_file is not None) or (json_file is not None):
                                    returned_records.extend(brt_enum_records)
                            else:
                        ...
                        srv_enum_records = self.brute_srv(res, domain, verbose)
                        ...
                        tld_enum_records = self.brute_tlds(res, domain, verbose)
                        if (output_file is not None) or (results_db is not None) or (csv_file is not None) or (json_file is not None):
                            returned_records.extend(tld_enum_records)

                    elif r == "goo":
                        print_status("Performing Google Search Enumeration against {0}".format(domain))
                        goo_enum_records = self.se_result_process(res, scrape_google(domain))
                        if (output_file is not None) or (results_db is not None) or (csv_file is not None) or (json_file is not None):
                        ...
                        crt_enum_records = self.se_result_process(res, scrape_crtsh(domain))
                        if (output_file is not None) or (results_db is not None) or (csv_file is not None) or (json_file is not None):
                        ...
                        if (dict is not None) and (ns_server is not None):
                            ...
                            cache_enum_records = self.in_cache(res, dict, ns_server)
                            if (output_file is not None) or (results_db is not None) or (csv_file is not None) or (json_file is not None):
                        ...
                        if (output_file is not None) or (results_db is not None) or (csv_file is not None) or (json_file is not None):
                            returned_records.extend(self.ds_zone_walk(res, domain))
                        else:
                            self.ds_zone_walk(res, domain)
            ...
            if (output_file is not None):
                ...
                xml_enum_doc = self.dns_record_from_dict(returned_records, scan_info, domain)
                self.write_to_file(xml_enum_doc, output_file)
            ...
            if (results_db is not None):
                ...
                self.create_db(results_db)
                self.write_db(results_db, returned_records)
            ...
            if (csv_file is not None):
                ...
                self.write_to_file(self.make_csv(returned_records), csv_file)
            ...
            if (json_file is not None):
                ...
                result = self.write_json(json_file, returned_records, scan_info)
                data = []
...
if __name__ == "__main__":
    #main()
    api.add_resource(Dnsrecon, "/dnsrecon/<string:target>")
