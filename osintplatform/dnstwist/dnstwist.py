...
def p_json(data):
	...
	if args.format == 'json':
		...
		for i in range(len(tab)):
			...
			tab[i] = tab[i].strip()
			#tab[i] = ast.literal_eval(tab[i])
			tab[i] = json.loads(tab[i])
...
def generate_json(domains):
	...
	return json.dumps(json_domains, sort_keys=True)
...
class Dnstwist(Resource):
	...
	def post(self, target):
		...
		args.mxcheck = True
		args.tld = "dictionaries/common_tlds.dict"
		args.format = "json"
		args.geoip = True
		args.domain = target
	...
	def get(self, target):
		...
		results.append(target_)
		return results, 200


if __name__ == '__main__':
	api.add_resource(Dnstwist, "/dnstwist/<string:target>")
