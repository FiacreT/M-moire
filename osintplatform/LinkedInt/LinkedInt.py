...
class LinkedInt(Resource):
     
    
    
    def post(self, target, bycompany, domain_surfix, prefix_email):
        global csv 
        global bCompany 
        global bAuto 
        global search 
    ...
    def get(self, target, bycompany, domain_surfix, prefix_email):
        result = []
        results = {}
        for i in range(len(csv)):
            info_personne = csv[i].split(",")
            result.append({"name":info_personne[2], "email":info_personne[3], "poste":info_personne[4], "localisation": info_personne[5]})
        results = {
            "info personne":result,
            "target": target
...
if __name__ == '__main__':
    api.add_resource(LinkedInt, "/linkedInt/<string:target>/<string:bycompany>/<string:domain_surfix>/<string:prefix_email>")
    app.run(debug=True, port=5012, threaded=True)
