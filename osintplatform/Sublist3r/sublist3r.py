...
class portscan():
    ...
    def port_scan(self, host, ports):
        ...
        if len(openports) > 0:
            ...
            ports_open.append(host + ":" + ', '.join(openports))
            #ports_open.append("%s%s%s - %sFound open ports:%s %s%s%s" % (G, host, W, R, W, Y, ', '.join(openports), W))
...
class Sublist3r(Resource):

    def post(self, target, ports_ = ""):
        ports_ = ports_.replace("_", ",")
        args = parse_args()
        domain = target
        ...
        return "Sublist3r", 201


    def get(self, target, ports_=""):
        subdomains = []
...
if __name__ == "__main__":
    #interactive()
    api.add_resource(Sublist3r, "/sublist3r/<string:target>")
